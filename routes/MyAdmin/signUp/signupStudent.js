...
router.post('/', async (req, res) => {
    const {mail_id, password, student_RegNO, Student_Name, Class_id, dept_id, year_id} = req.body;
    let student_id = null;
    try {
        // Hash the password
        ...
        if (loginResult && loginResult[0].insertId) {
            ...
            const studentInsertQuery = `
                ...
                VALUES(?,?,?,?,?,?)
            `;
            await connection.query(studentInsertQuery, [student_id, student_RegNO, Student_Name, Class_id, dept_id, year_id]);

            // Send a success response
            res.status(200).send('User registered successfully');
        } else {
            // If loginResult.insertId is not available, handle the error
            throw new Error('Failed to get student_id after inserting into Student_login_table');
        }
    }catch (error) {
